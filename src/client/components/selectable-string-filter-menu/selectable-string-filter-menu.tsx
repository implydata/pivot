...
export class SelectableStringFilterMenu extends React.Component<SelectableStringFilterMenuProps, SelectableStringFilterMenuState> {
  ...
  componentWillMount() {
    var { essence, dimension, onSelectFilterOption } = this.props;
    ...
    if (colors) {
      ...
      if (filterMode) onSelectFilterOption(filterMode);
    }
  }
  ...
  renderRows() {
    var { loading, dataset, fetchQueued, searchText, selectedValues, promotedValues } = this.state;
    var { dimension, filterMode } = this.props;
    ...
    return <div className={classNames("string-filter-menu", filterMode)}>
      ...
      <div className={classNames('menu-table', hasMore ? 'has-more' : 'no-more')}>
        <StringFilterMenuControls
          ...
          filterMode={filterMode}
          placeholder="Search"
          noWorkToDo={!fetchQueued && !loading && dataset && dataset.data.length < TOP_N}
