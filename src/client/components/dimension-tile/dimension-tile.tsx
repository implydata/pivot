...
import { Clicker, Essence, VisStrategy, Dimension, SortOn, SplitCombine,
  ...
  granularityToString, getDefaultGranularityForKind, getGranularities, NEVER } from '../../../common/models/index';
...
export class DimensionTile extends React.Component<DimensionTileProps, DimensionTileState> {
  ...
  fetchData(essence: Essence, dimension: Dimension, sortOn: SortOn, unfolded: boolean, selectedGranularity?: Granularity): void {
    ...
    if (dimension.isContinuous() && dimension.bucketingStrategy !== NEVER) {
  ...
  render() {
    ...
    return <SearchableTile
      ...
      icons={icons}
      className={className}
      actions={(continuous && dimension.bucketingStrategy !== NEVER) ? this.getGranularityActions() : null}
      >
      <div className="rows">
        {rows}
        {message}
